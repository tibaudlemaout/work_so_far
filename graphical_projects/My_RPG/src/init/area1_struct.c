...
void area1_set_again_some_sprites(struct_t *data)
{
    sfTexture *bg = sfTexture_createFromFile("asset/1area.png", NULL);
    sfTexture *overlay = sfTexture_createFromFile("asset/1area3D.png", NULL);
    sfVector2f pos = {217, 470};
...
{
    area1_set_again_some_sprites(data);
    data->map = 1;
    data->area = 1;
    data->old_set = 14;
    get_collision_tab(data, "script/Collision1_2.txt");
    init_area_1_npc(data);
    sfText_setColor(data->interact_txt.txt, sfBlack);
}
