...
static int sort_meeting_name_alloc(employee_t *current, int count,
    employee_t **emp)
{
    while (current) {
        count++;
    ...
    employee_t *current)
{
    ...
    qsort(employees, count, sizeof(employee_t *), compare_by_name_emp);
}

static void sort_meeting_name_end(employee_t *prev, employee_t *sorted,
...
void sort_emp_list_by_name(employee_t **emp)
{
    ...
    count = sort_meeting_name_alloc(current, count, emp);
    ...
    sort_meeting_name_end(prev, sorted, emp, employees);
}
