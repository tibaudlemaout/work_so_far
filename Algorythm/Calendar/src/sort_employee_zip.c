...
static int sort_meeting_zip_alloc(employee_t *current, int count,
    employee_t **emp)
{
    while (current) {
        count++;
    ...
    employee_t *current)
{
    ...
    qsort(employees, count, sizeof(employee_t *), compare_by_location_emp);
}

static void sort_meeting_zip_end(employee_t *prev, employee_t *sorted,
...
void sort_emp_list_by_zip(employee_t **emp)
{
    ...
    count = sort_meeting_zip_alloc(current, count, emp);
    ...
    sort_meeting_zip_end(prev, sorted, emp, employees);
}
