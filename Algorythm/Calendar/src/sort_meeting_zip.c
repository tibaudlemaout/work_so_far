...
int sort_meeting_zip_alloc(meeting_t *current, int count, meeting_t **meet)
{
    while (current) {
        count++;
    ...
    meeting_t *current)
{
    ...
    qsort(meetings, count, sizeof(meeting_t *), compare_by_location);
}

void sort_meeting_zip_end(meeting_t *prev, meeting_t *sorted,
...
void sort_meeting_list_by_location(meeting_t **meet)
{
    ...
    count = sort_meeting_zip_alloc(current, count, meet);
    ...
    sort_meeting_zip_end(prev, sorted, meet, meetings);
}
