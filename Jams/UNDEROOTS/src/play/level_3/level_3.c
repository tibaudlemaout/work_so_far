...
void draw_level_3(struct_t *all, sfRenderWindow *win, sprite_t *script)
{
    ...
    for (int i = 0; i < 6; i++) {
        if (all->kills == 1 && script[i].life != 0 && i != 5 && i != 4 && i != 3) {
            sfRenderWindow_drawSprite(win, script[i].sprite, NULL);
            script[i].display = 1;
            script[i].delay = 1;
        }
        if (all->kills == 2 && script[i].life != 0 && i != 5 && i != 4) {
            sfRenderWindow_drawSprite(win, script[i].sprite, NULL);
            script[i].display = 1;
            script[i].delay = 1;
        }
        if (all->kills == 3 && script[i].life != 0 && i != 5) {
            sfRenderWindow_drawSprite(win, script[i].sprite, NULL);
            script[i].display = 1;
            script[i].delay = 1;
        }
        if (all->kills > 3 && script[i].life != 0) {
            sfRenderWindow_drawSprite(win, script[i].sprite, NULL);
            script[i].display = 1;
            script[i].delay = 1;
        }
...
void manage3(struct_t *all, sfEvent *event, sfRenderWindow *win, sprite_t *script)
{
...
void level_3(struct_t *all, sfEvent *event)
{
    ...
    while (all->set == 3 && all->level == 3) {
        ...
        draw_level_3(all, WIN, script);
        anim_monsters3(all, WIN, script, clock);
