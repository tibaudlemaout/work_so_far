...
void draw_level_1(struct_t *all, sfRenderWindow *win, sprite_t *script)
{
    ...
    for (int i = 0; i < 4; i++) {
        if (all->kills == 1 && script[i].life != 0 && i != 3) {
...
void manage1(struct_t *all, sfEvent *event, sfRenderWindow *win, sprite_t *script)
{
...
void level_1(struct_t *all, sfEvent *event)
{
    ...
    while (all->set == 3 && all->level == 1) {
        sfRenderWindow_display(WIN);
        sfRenderWindow_clear(WIN, sfBlack);

        draw_level_1(all, WIN, script);
        anim_monsters1(all, WIN, script, clock);
